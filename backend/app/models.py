...
class Jewelry(Base):
    """ORM model representing a jewelry item with its 3D model metadata."""

    __tablename__ = "jewelry"

    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    name = Column(String(200), nullable=False, index=True)
    category = Column(String(100), nullable=False, default="ring", index=True)
    brand = Column(String(100), nullable=True)
    description = Column(Text, nullable=True)
    ...
    # 3D Transform — Rotation offset (degrees, converted to radians on client side)
    rotation_offset_x = Column(Float, nullable=False, default=0.0)
    rotation_offset_y = Column(Float, nullable=False, default=0.0)
    rotation_offset_z = Column(Float, nullable=False, default=0.0)
